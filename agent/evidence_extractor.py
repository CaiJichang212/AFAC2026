...
def _build_fact_matrix_messages(
    *,
    ...
    system_prompt = (
        ...
        "8. **重要**：除了从文档页面提取规则外，还必须从**问题题干**中提取每个产品的具体数值。"
        "例如题干中提到\"平安智盈金生保单账户价值90万元\"，"
        "则应额外输出 {product: \"平安智盈金生\", field: \"保单账户价值\", formula_or_value: \"900000\", unit: \"元\"}。"
        "题干中的每个产品数值（账户价值、基本保额、已领养老年金等）都要作为独立事实输出。"
        "这些数值事实对于正确计算公式结果至关重要。\n\n"
